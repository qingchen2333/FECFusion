# -*- coding: utf-8 -*-
"""
P.S.:基于原作者代码，添加批量生成功能，但要求是同一源图像对的不同算法的结果图像
...
import cv2
import numpy as np
from my_utils.dataset_util import prepare_data_path
...
def MultiRegion():
    file_dir = r'../results/NeedPlot/'
    save_dir = r'../results/PlotResults'
    data, filenames = prepare_data_path(file_dir)
    # 设置框图坐标
    region_list = [
        [137, 105, 167, 161],       # 顺序对应（x1,y1）(x2,y2)的框图
        [149, 171, 172, 212]
    ]
    ...
    for i in range(len(data)):
        save_name = 'plot_{}'.format(filenames[i])
        save_path = os.path.join(save_dir, save_name)
        plotMultiRegion(data[i], region_list=region_list, line_width_list=line_list, color_list=color_list,
                        place_list=place_list, scale_list=scale_list, save_path=save_path, zoom_bool=zoom_bool)
