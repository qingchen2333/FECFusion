...
from my_utils.my_args import get_ablation_args
...
def test(num, deploy_flag=True):
    # 一、初始化配置
    # 1.1 配置超参
    args = get_ablation_args()
    # 1.2 配置设备
    ...
    print('Loading {} ...'.format(fusion_model_file))
    if deploy_flag:
        ModelNet = model_deploy(ModelNet)

    datasets = ['MSRS']      # 'TNO', 'RoadScene', 'MSRS', 'M3FD'
    for dataset in datasets:
        ...
        fused_path = os.path.join('results', 'ablation', str(E_num) + '_epo' + str(num), dataset)
        ...
        print('Dataset size is {}'.format(test_loader.n_iter))
        ...
        # 二、融合步骤
        ModelNet.eval()
        x = torch.randn(1, 1, 480, 640).to(device)
        ...
        with torch.no_grad():
            for it, (images_vis, images_ir, name, h, w) in enumerate(test_loader):
                ...
                print('Fusion {}, Run time is {}s'.format(save_path, run_time))
